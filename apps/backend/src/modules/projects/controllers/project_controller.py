...
def response(access: ProjectAccess) -> ProjectResponse:
    ...
    if access.role.value == "owner":
        permissions = {
            feature: {"view": True, "manage": True}
            for feature in ProjectService.FEATURES
        }
...
async def list_project_members(
    ...
    try:
        return ProjectMemberListResponse(
            members=[
                member_response(item)
                for item in await service.members(project_id, user.id)
            ]
        )
    except (ProjectNotFoundError, ProjectPermissionError) as error:
        raise translate(error) from None
...
async def update_project_member(
    project_id: UUID,
    member_id: UUID,
    body: UpdateProjectMemberAccessRequest,
    user: Annotated[AuthenticatedUser, Depends(current_user)],
    service: Annotated[ProjectService, Depends(project_service)],
):
    try:
        await service.update_member_access(
            project_id, user.id, member_id, body.permissions
        )
    except ValueError as error:
        raise HTTPException(400, str(error)) from None
    except (ProjectNotFoundError, ProjectPermissionError) as error:
