...
async def create_conversation(
    ...
    try:
        chat, role = await service.create_for_assistant(
            assistant_id, user.id, body.title
        )
...
@router.get("/assistants/{assistant_id}/conversations", response_model=ChatListResponse)
    ...
    async def events():
        try:
            async for event in service.stream_message(chat_id, user.id, body.content):
                if event["type"] == "done":
                    event["message"] = message_response(event["message"]).model_dump(
                        mode="json"
                    )
