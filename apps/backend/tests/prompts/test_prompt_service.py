...
class PromptValidationTests(unittest.TestCase):
    def setUp(self):
        self.service = PromptService(Repository())

    def test_variables_are_inferred_deterministically(self):
        variables, digest = self.service._validate(
            "rag_answer", "Answer {{question}} from {{ context }} and {{context}}"
        )
        self.assertEqual(["context", "question"], variables)
