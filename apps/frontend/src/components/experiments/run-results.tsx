...
export function RunResults({
	...
	const selectedVariant =
		detail.variants.find(item => item.id === chosenVariantId) ??
		detail.variants[0];
	const variantCases = detail.cases.filter(
		item => item.variant_run_id === selectedVariant?.id,
	);
	const [chosenCaseId, setChosenCaseId] = useState(
		variantCases[0]?.id ?? '',
	);
	const selectedCase =
		variantCases.find(item => item.id === chosenCaseId) ??
		variantCases[0];
	const expectedTotal =
		expectedCases * Math.max(detail.variants.length, 1);
	const failedCount = Math.max(expectedTotal - detail.cases.length, 0);
	const isComparison =
		detail.run.status === 'comparison' ||
	...
	return (
		<section
			...
			aria-label='Experiment run results'>
			<header className='results-header'>
				<div>
					<h3>
						{isComparison
							? 'Variant comparison'
							: `${detail.variants[0]?.name ?? 'Run'} results`}
					</h3>
					<p>
						{isComparison
							? 'The same benchmark measured across selected variants.'
							: 'Benchmark outcome and individual case evidence.'}
					</p>
				</div>
				{!isComparison && (
					<span
						className={`catalog-state ${detail.run.status}`}>
						{detail.run.status}
					</span>
				)}
			</header>
			<div className='run-outcome-grid'>
				...
				<Summary
					...
					value={`${detail.cases.length} / ${expectedTotal}`}
					tone={
						failedCount
							? 'warning'
							: 'positive'
				...
				<Summary
					...
					value={definition(primaryMetric).label}
				/>
			</div>
			{errors.length > 0 && (
				<details className='run-error-summary'>
					<summary>
						{errors.length} execution{' '}
						{errors.length === 1
							? 'error'
							: 'errors'}
					</summary>
			...
			<section className='results-section'>
				<header>
					<div>
						...
						<p>
							{isComparison
								? 'Best values are highlighted. Quality is higher-is-better; latency, tokens, and cost are lower-is-better.'
								: 'Aggregate scores across completed cases.'}
						</p>
					</div>
				</header>
								...
								return (
									<tr
										...
										<th scope='row'>
											...
											{metric ===
												primaryMetric && (
												<span className='primary-metric-label'>
													Primary
												</span>
											)}
										</th>
			...
			<section className='results-section case-explorer'>
				<header>
					<div>
						<h4>Benchmark cases</h4>
						<p>
							Select a case to inspect
							its answer, scores, and
							evaluator notes.
						</p>
					</div>
				</header>
				{detail.variants.length > 1 && (
					<div
						className='result-variant-tabs'
						role='tablist'
						aria-label='Result variants'>
						{detail.variants.map(
							variant => (
								<button
									type='button'
										...
										<tr
											...
											<td>
												<button
													...
													aria-label={`Inspect benchmark case ${item.position + 1}`}
													onClick={() =>
														setChosenCaseId(
															item.id,
														)
...
function Summary({
	...
	return (
		<div className={`run-outcome ${tone}`}>
			<span>{label}</span>
			<strong>{value}</strong>
		</div>
	);
...
function CaseInspection({ result }: { result: CaseResult }) {
	return (
		<article className='case-inspection'>
			<header>
				<span>Case {result.position + 1}</span>
			...
			<div className='case-metrics-table'>
				<table>
					<caption className='sr-only'>
						Metrics for benchmark case{' '}
						{result.position + 1}
					</caption>
					<thead>
						<tr>
						...
						{Object.entries(
							...
							return (
								<tr
									...
									<td>
										<span
											className={`metric-rating ${display.tone}`}>
											{
												display.rating
											}
										</span>
...
function MetricValue({ metric, value }: { metric: string; value: unknown }) {
	...
	return (
		<span className='aggregate-metric-value'>
			...
			<span className={`metric-rating ${display.tone}`}>
				{display.rating}
			</span>
		</span>
	);
...
function bestValue(metric: string, variants: VariantResult[]) {
	const values = variants
		.map(variant => variant.aggregate_metrics?.[metric])
		.filter(
	...
	return ['latency', 'tokens', 'cost'].includes(definition(metric).kind)
		? Math.min(...values)
		: Math.max(...values);
}

function scoreValue(value: unknown) {
	return isScoredMetric(value) ? value.score : value;
}
...
function cleanProviderError(error: string) {
	const withoutHelpLinks = error.replace(
		/For more information check:\s*https?:\/\/\S+/g,
		'',
	);
	return [
		...new Set(
...
function metricDisplay(metric: string, value: unknown) {
	...
	if (config.kind === 'boolean')
		return {
			value: value === true ? 'Hit' : 'Miss',
			unit: '—',
			rating: value === true ? 'Efficient' : 'Uncached',
			tone: value === true ? 'excellent' : 'neutral',
		};
