...
export function ExperimentManager({ project, initialCatalog }: { project: Project; initialCatalog: ExperimentCatalog }) {
	...
	return <div className='experiment-page'>
		...
		<section className='catalog-table-panel'><header><div><h2>Experiment registry</h2><p>{catalog.experiments.length} experiments</p></div></header><div className='catalog-table-wrap'><table><thead><tr><th>Experiment</th><th>Benchmark</th><th>Primary metric</th><th>Variants</th><th>Status</th><th /></tr></thead><tbody>{catalog.experiments.map(experiment => <tr key={experiment.id} onClick={() => inspect(experiment.id)}><td><span className='catalog-primary'><FlaskConical size={14} /><strong>{experiment.name}</strong></span></td><td>{experiment.benchmark_name} · v{experiment.benchmark_version}</td><td>{metricLabel(experiment.primary_metric)}</td><td>{experiment.variant_count}</td><td><span className={`catalog-state ${experiment.status}`}>{experiment.status}</span></td><td><ChevronRight size={14} /></td></tr>)}</tbody></table>{!catalog.experiments.length && <div className='mock-empty'>No experiments created yet.</div>}</div></section>
		{createOpen && <ExperimentForm catalog={catalog} busy={busy} submit={create} close={() => setCreateOpen(false)} />}
		{detail && <div className='index-detail-backdrop'><section className='experiment-detail-panel'>
			<header><div><span className='eyebrow'>Experiment</span><h2>{detail.experiment.name}</h2><p>{detail.experiment.description || 'No description'}</p></div><button aria-label='Close experiment' onClick={() => { setRunDetail(null); setSelectedVariant(null); setDetail(null); }}><X size={18} /></button></header>
			...
			<nav className='experiment-panel-tabs' aria-label='Experiment sections'>{(['variants', 'runs', 'results'] as PanelTab[]).map(tab => <button key={tab} className={activeTab === tab ? 'active' : ''} onClick={() => setActiveTab(tab)} disabled={tab === 'results' && !runDetail}>{tab === 'variants' ? `Variants (${detail.variants.length})` : tab === 'runs' ? `Runs (${detail.runs.length})` : 'Results'}</button>)}</nav>
			...
			{activeTab === 'results' && (runDetail ? <RunResults detail={runDetail} metrics={detail.experiment.metrics} /> : <div className='mock-empty experiment-results-empty'>Select View on a run to inspect its results.</div>)}
